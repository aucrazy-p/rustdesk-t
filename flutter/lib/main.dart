...
/// Force hide the connection manager (CM) window.
/// When true, the CM process runs normally (handling incoming connections
/// via IPC) but its window is never shown — not at startup, not on incoming
/// connections, not on chat messages. Set to false to restore default behavior.
const kForceHideCm = true;
...
showCmWindow({bool isStartup = false}) async {
  // Force hide: the CM window must never be shown.
  // All callers (connection events, chat messages, timer callbacks) are
  // silently ignored so no window pops up during connections.
  if (kForceHideCm) return;
  if (isStartup) {
    WindowOptions windowOptions = getHiddenTitleBarWindowOptions(
